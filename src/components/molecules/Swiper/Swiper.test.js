...
describe("Swiper Component", () => {
    test("renders product inclusion slides", () => {
        ...
        fireEvent.touchStart(imgsEl[2], { touches: [{ clientX: 347, clientY: 534 }] });
        fireEvent.touchMove(imgsEl[2], { touches: [{ clientX: 134, clientY: 534 }] });
        const swiperContent = screen.getByTestId("swiper-content");

        expect(swiperContent.style.transform).toEqual("translateX(-75%)");
        fireEvent.touchStart(imgsEl[2], { touches: [{ clientX: 134, clientY: 534 }] });
        fireEvent.touchMove(imgsEl[2], { touches: [{ clientX: 347, clientY: 534 }] });
