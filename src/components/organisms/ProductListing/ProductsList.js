...
export default function ProductsList({
  ...
  const { products, sortBy, isLoading, total } = useSelector(
  ...
  return (
    <>
      {isLoading && <Loader />}
      {products?.length === 0 && searchStr !== "" && (
        <div className="products-container">
          <div className="products-count">No product found!</div>
        </div>
      )}
      {searchStr === "" && loadedFromSearch && products?.length === 0 &&(
        <div className="products-container">
      ...
      <div className="products-container">
        ...
        {items.length > 0 && (
          <div className="products-count">
            Viewing {items.length} of {total} products
          </div>
        )}
